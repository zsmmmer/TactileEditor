...
function drawLines() {
  ...
  lines.forEach((line, index) => {
    ...
    drawVertex(line.x1, line.y1, selectedVertex === `${index}-start`);
    drawVertex(line.x2, line.y2, selectedVertex === `${index}-end`);
  });
}

function drawVertex(x, y, isSelected) {
  ctx.beginPath();
...
canvas.addEventListener("mousedown", (event) => {
  ...
  lines.forEach((line, index) => {
    if (isNearVertex(line.x1, line.y1, mouseX, mouseY)) {
      selectedVertex = `${index}-start`;
    } else if (isNearVertex(line.x2, line.y2, mouseX, mouseY)) {
      selectedVertex = `${index}-end`;
    } else if (isNearLine(line, mouseX, mouseY)) {
      selectedLine = index;
    }
  });
...
canvas.addEventListener("mousemove", (event) => {
  if (isDragging) {
    ...
    if (selectedVertex) {
      const [index, vertex] = selectedVertex.split("-");
      const line = lines[parseInt(index)];
...
function isNearVertex(x1, y1, x2, y2) {
